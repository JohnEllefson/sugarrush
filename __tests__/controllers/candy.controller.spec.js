  ...
  ******************************************/
  describe('getAllCandy', () => {
    ...
    test('should return 404 if no matching candy items found', async () => {
      ...
      expect(res._getJSONData()).toEqual({ message: 'No matching candy items found' });
    });

    test('should apply valid description filter', async () => {
      const mockCandy = [{ _id: '2', name: 'Gum', description: 'Chewy and sweet' }];
      Candy.find.mockResolvedValue(mockCandy);
    ...
    test('should apply valid container filter', async () => {
      const mockCandy = [{ _id: '3', name: 'Hard Candy', shipping_container: 'Tin' }];
      Candy.find.mockResolvedValue(mockCandy);
  ...
  ******************************************/
  describe('deleteSingleCandy', () => {
    test('should delete a candy item by ID', async () => {
      Candy.findByIdAndDelete.mockResolvedValue({ _id: '456', name: 'Sour Patch' });
      ...
      expect(res.statusCode).toBe(200);
      expect(res._getJSONData()).toEqual({ message: 'Candy item deleted successfully' });
    });
  ...
  ******************************************/
  describe('createSingleCandy', () => {
    test('should create a new candy item', async () => {
      ...
      expect(res._getJSONData()).toEqual({ message: 'New candy item added', id: 'new123' });
    });

    test('should handle errors when creating candy item', async () => {
      jest.spyOn(Candy.prototype, 'save').mockRejectedValue(new Error('Create failed'));
